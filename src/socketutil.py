...
@socketio.on('startTrace_button', namespace='/defchishi')
def startTrace():
    # logger.info("startTrace_button called")
    content = {
        'pkgname': genv.get_pkgname(),
        'flagSecure': 0
    }
    ...
    try:
        ...
        logger.error("startTrace error: %s" % e)

@socketio.on('stopTrace_button', namespace='/defchishi')
def stopTrace():
    logger.info("[*] stop Trace!")
    genv.script.unload()
...
@socketio.on('unloadToBurpScript', namespace='/defchishi')
def unload_to_burp_script():
    logger.info("unload_to_burp_script")
    if genv.toBurpScript :
        try:
            genv.toBurpScript.unload()
        except Exception as e:
            logger.warning("The Script has unloaded!")
...
def detachall():
    ...
    # toBurpSession
    try:
        if genv.toBurpSession is not None:
            genv.toBurpSession.detach()
        else:
            logger.warning("No toBurpScripts can be unloaded.")
    except Exception as e:
        logger.error("detach all Fail, Reason is {}.".format(e))
...
def load_custom_script(message):
    ...
    try:
        if type == "normal":
            if '-to0obuooO0rp-' in ScriptContent:
...
@socketio.on('refreshPid', namespace='/defchishi')
def refreshPid():
    try:
        rdev = genv.get_device()
        if rdev is None:
...
def generate_export_instance(message):
    ...
    for item in methods_list:
        ...
        platform = item.get('platform')
        if "IOS" == platform:
            content = {'scripts': "", "iosscript":iosscript_content}
