...
function UnloadToBurpScript() {

    socket.emit("unloadToBurpScript");
}

function deleteToBurpInfo(){

    var lists = toBurpinfo.getValue().split('\n');
    for (let i = 0; i < lists.length; i++) {

        var isTpBurpMethod_str = lists[i];
        var isTpBurpMethod_json = JSON.parse(isTpBurpMethod_str);

        if (isTpBurpMethod_json.hasOwnProperty("argNum")){
            delete isTpBurpMethod_json.argNum;                
            delete isTpBurpMethod_json.methodNum;
            delete isTpBurpMethod_json.inputStr;
            delete isTpBurpMethod_json.modeNum;
        }
        isTpBurpMethod_str = JSON.stringify(isTpBurpMethod_json);
        lists.splice(i, 1, isTpBurpMethod_str);
        var toBurpInfo_str = lists.join('\n');
        toBurpinfo.setValue(toBurpInfo_str);
    }
...
// argNum             要通过第几个参数判断，第一个传 1
// modeNum            参数判断 0； 返回值判断 1； 参数判断&拦截返回值 2； twostep判断函数 3； twostep修改函数 4；
// inputStr           要判断的字符串
// 仅兼容 iOS 平台
function addIsToBurpInfo(arg_list) {
    ...
    for (let i = 0; i < arg_list.length; i++) {
        ...
        methodNum = arg_json['methodNum'];
        argNum = arg_json['argNum'];
        modeNum = arg_json['modeNum'];
        inputStr = arg_json['inputStr'];
        

        var lists = toBurpinfo.getValue().split('\n');

        var isTpBurpMethod_str = lists[methodNum-1];
        var isTpBurpMethod_json = JSON.parse(isTpBurpMethod_str);
        isTpBurpMethod_json.argNum = argNum;
        isTpBurpMethod_json.modeNum = modeNum;
        isTpBurpMethod_json.inputStr = inputStr;
...
window.onload = function() {
    ...
    $("#startTrace").click(function(){
        socket.emit("startTrace_button")
    })

    $("#stopTrace").click(function(){
        socket.emit("stopTrace_button")
    })

    $("#clearTraceInfo").click(function(){
        $("#uiNetwork").empty()
    })
    ...
    socket.on('OutputGenerateExportToburpScript', function(msg) {
        var filecontent = msg.filecontent;
        Customcode.setValue(filecontent)
        // ScriptManageEditorView.setValue(filecontent);
        // console.log(filecontent);
    ...
    //Handle backtrace.js
    socket.on('networkBacktrace', function(msg){
        // $('#uiNetwork').append('<div style="font-size:15px; white-space:pre">' + msg.data + '</div>')
        $('#uiNetwork').append('<pre style="font-size:12px;">' + msg.data + '</pre>')
    })
    ...
    socket.on('findhook_message', function(msg) {
        ...
        if(isJSON(msg.data)) {
            ...
            f_data = JSON.parse(msg.data);
        } else {
            var str = HTMLDecode(msg.data);
            f_data = JSON.parse(str);
        }
    ...
    socket.on('getapp', function(msg) {
        ...
        for (var i = 0;i < arr.length;i++){
            var jsondata = arr[i];
            if(jsondata.pid !=0)
                $('#outputappname').append('<tr><td>' + jsondata.pkgname + '</td><td><code>' + jsondata.name + '</code></td><td>' + jsondata.pid + '</td><td><button id=\"' + jsondata.pkgname + '\" class="btn btn-success " style="width: 90px;height: 32px;margin-bottom: 2px" onclick="selectApp(this.id)">Select</button></td></tr>');
        }
