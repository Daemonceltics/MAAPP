...
def on_message(message, data):
    ...
    if message['type'] == 'send':
        ...
        if "-h00oOOoks-" in info:
            ...
            genv.toBurpScript.post({'type': 'input', 'payload': req.text})
            ...
            socketio.emit('findhook_message',
                          {'data': html.escape(json.dumps(j_info))},
                          namespace='/defchishi')
        ...
        elif "networkRequestBacktace" in info:
            # logger.info(info)
            socketio.emit('networkBacktrace',
...
def loadToBurpScript(script_content):
    try:
        ...
        # print("loadScript:\n" + script_content)
        time.sleep(1)
        genv.toBurpScript = genv.toBurpSession.create_script(script_content)  # 注册js代码
        logger.info("script_on...")
        genv.toBurpScript.on("message", on_message)  # on()方法注册message handler
        genv.toBurpScript.load()
