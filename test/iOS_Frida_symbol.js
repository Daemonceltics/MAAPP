...
function find_symbol_from_addressX(modulePath,addr){
    var frameAddr = addr
    ...
    for(var i = 0, len = allClass.length; i < len; i++){
        ...
        if(done)
            break;
        //done==1 ? break:continue;
        // XLOG("theClass:"+theClass + "  " + "theMethodName:" +theMethodName);
        // if(theClass != "Node" && theMethodName != "None")
        //     break;
    }
...
function backtrace(onlyMainModule){
    ...
    // NSArray<NSString *>
    var symobls = threadClass["+ callStackSymbols"]()
